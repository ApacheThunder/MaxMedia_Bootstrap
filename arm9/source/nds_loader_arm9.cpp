...
static bool dldiPatchLoader (data_t *binData, u32 binSize, bool clearBSS, bool useSlot2DLDI) {
...
eRunNdsRetCode runNdsFile (const char* filename, int argc, const char** argv)  {
	...
	if(argv[0][0] == 's' && argv[0][1] == 'l' && argv[0][2] == 'o' && argv[0][3] == 't' && argv[0][4] == '2')useSlot2 = true;

	return runNds (load_bin, load_bin_size, st.st_ino, true, useSlot2, argc, argv);
}
